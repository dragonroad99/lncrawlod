...
class WwwNovelupdateClub(Scraper):
    ...
    def fetch_info(self, ctx: Context) -> None:
        ...
        # Parse novel
        ctx.novel.name = SoupUtils.select_value(soup, "h1.widget-title", attr="text")
        ctx.novel.name = TextUtils.ascii_only(ctx.novel.name)

        ctx.novel.cover_url = SoupUtils.select_value(soup, "img.lazyload.img-fluid.p-0.mb-3", attr="data-src")
        ctx.novel.details = str(soup.select_one("div[itemprop='description']")).strip()
        ...
        for serial, a in enumerate(soup.select("section div.container div.row div.col-lg-8.pl-0 div.post div.row a.col-md-6")):
    ...
    def fetch_chapter(self, ctx: Context, chapter: Chapter) -> None:
        ...
        body = soup.select("section div.container div.post.post-single p")
